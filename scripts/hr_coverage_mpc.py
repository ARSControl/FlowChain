...
for s in range(NUM_STEPS):
  ...
  # human
  # ax.scatter(human_traj[s+1, 0], human_traj[s+1, 1], marker='x', color='tab:purple', label='Human Start')
  # overall (pdf + human)
  Z_tot = np.clip(Z - alpha_human*human_pdf, 1e-6, None)
  # Z_tot /= Z_tot.sum()
  # coords = np.stack([X.ravel(), Y.ravel()], axis=1)
  # samples_weights = Z_tot.ravel()
  # num_samples = 1000
  # indices = np.random.choice(len(coords), size=num_samples, p=samples_weights)
